  ...
  <div class="archives">
    ...
    <div class="m-archives">
      ...
      <el-card class="blog-container">
        ...
        <timeline-item bg-color="#9dd8e0" v-for="item of archiveList" :key="item.Id">
          <!-- 日期 -->
...
import archives from '@/api/info/archives'
export default {
  name: 'archive',
  components: {
    Timeline,
  ...
  methods: {
    toBlog (blogId) {
      sessionStorage.setItem('id', blogId)
      this.$router.push('/blog')
    },
    async getArchiveList () {
      archives.getArchives()
        .then(res => {
            this.archiveList = res.list.records
