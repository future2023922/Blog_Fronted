...
<!--              文章来源，标题，分类-->
              <div class="inputContent">
                ...
                <el-form-item prop="copyright">
                  <el-select v-model="formData.copyright" placeholder="原创" style="margin-right: 10px; width: 100px;">
                    <el-option
...
<!--              发布博客-->
              <div class="release">
                <el-button round @click="saveOrUpdate" style="margin-top: 30px;background-color: rgba(64,158,255,0.8);color: white">发布</el-button>
              </div>
  ...
  methods: {
    ...
    async getTypeList () {
      category.getCategoryList().then(response => {
        this.typeList = response.categoryList
      //前端如何发送request
      }).catch(error => {
        console.log(error)
      })
    ...
    edit() {
      this.$refs.addForm.validate(
        (valid) => {
          if (valid) {
            blog.updateBlog(this.formData).then(response => {
              this.$message({
                type: "success",
                message: "修改成功"
              });
