...
<!--        </router-link>-->
      ...
      <div class="float-right nav-title">
        ...
        <div class="menus-item">
          <router-link to="/home">
            <i class="iconfont2 iconzhuye" style="color: whitesmoke" /><span style="color: whitesmoke;margin-right: 10px">博客</span>
          </router-link>
        </div>
        <div class="menus-item">
        ...
        <div class="user-btn">
          <a>
          <router-link to="/">
            <i class="iconfont2 icondenglu" style="color: whitesmoke"/><span style="color: whitesmoke;margin-right: 10px">{{info.name ? `用户： ${info.name}` : '未登录'}}</span>
            </router-link>
          </a>
...
<script>
  ...
  methods: {
    toLogin () {
      ...
      // 后端指定接口验证了token的正确性
      if (!tokenStr) {
        this.$confirm('登录后才能进入后台，请问是否先登录？', '提示', { // 确认框
          type: 'info'
        }).then(() => {
